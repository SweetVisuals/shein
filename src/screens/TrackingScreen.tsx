        ...
        <div className="bg-[#f5f5f5] min-h-screen">
          ...
          <div className="bg-white px-4 pt-3 border-b border-gray-200">
             ...
             <div className="flex gap-2 py-3 overflow-x-auto no-scrollbar">
                {shipment?.order_items?.map((item: any) => (
                   <div key={item.id} className="w-[72px] h-[96px] flex-shrink-0 bg-gray-100 rounded-sm overflow-hidden border border-gray-100">
          ...
          <div className="bg-white p-5 mt-2">
             {events.length > 0 ? events.map((event, index) => (
                <div key={event.id} className={`relative border-l-2 ${index === 0 ? 'border-[#188a42]' : 'border-gray-200'} ml-4 pb-8 last:pb-0 last:border-transparent`}>
                   {index === 0 ? (
                      <div className="absolute -left-[11px] top-0 w-5 h-5 rounded-full bg-[#188a42] flex items-center justify-center text-white border-2 border-white shadow-md">
                   ...
                   <div className="pl-6">
                      <div className={`${index === 0 ? 'font-bold text-[#188a42]' : 'text-gray-800 font-medium'} text-sm mb-0.5 leading-tight`}>
                         {event.event_type}<br/>{event.location}
                      </div>
                      <div className="text-gray-400 text-xs">{new Date(event.occurred_at).toLocaleString()}</div>
                   </div>
                </div>
             )) : (
               <div className="relative border-l-2 border-[#188a42] ml-4 pb-8">
                  ...
                  <div className="pl-6">
                     <div className="font-bold text-[#188a42] text-sm mb-0.5 leading-tight">Order Placed<br/>System Processing</div>
                     <div className="text-gray-400 text-xs">Awaiting tracking update</div>
                  </div>
               </div>
             )}
          </div>
